    ...
    def terminal_nodes(self, chunk):
        ...
        for node in chunk:
            ...
            if node.getLemma()=='((':
                ...
                # node_name=str.replace(node_name, 'ठ', 'प')
    ...
    def fragments(self):
        ...
        for chunk in self.tree.getChildren(self.Pred_P):
            ...
            if (Chunk_PC=='VGF' or Chunk_PC=='NULL__VGF' or Chunk_PC=='VGNN' or Chunk_PC=='VGNF' or Chunk_PC=='NULL__VGNN' or Chunk_PC=='NULL__VGNF'):
                ...
                # Tanya- changed dependency relations here for Universal Dependency
                # if (chunk.isChild('nsubj', self.Pred_P) or chunk.isChild('nsubj_    pass', self.Pred_P)):
                #     Frag = " [." + Chunk_PC + '-SUBJ' + child_f + " ]"
                #     all_dict_frag[Frag] = Chunk_Posn
                #
                # elif chunk.isChild('iobj', self.Pred_P):
                #     Frag = " [." + Chunk_PC + '-OBJ-2' + child_f + " ]"
                #     all_dict_frag[Frag] = Chunk_Posn
                #
                # elif chunk.isChild('nmod', self.Pred_P):
                #     Frag = " [." + Chunk_PC + '-J-SUBJ' + child_f + " ]"
                #     all_dict_frag[Frag] = Chunk_Posn
                #
                # elif (chunk.isChild('dobj', self.Pred_P) or chunk.isChild('obj', self.Pred_P) or chunk.isChild('xcomp', self.Pred_P) or chunk.isChild('ccomp', self.Pred_P)):
                #     Frag = " [." + Chunk_PC + '-OBJ-1' + child_f + " ]"
                #     all_dict_frag[Frag] = Chunk_Posn
                #
                # elif chunk.isChild('acl', self.Pred_P):
                #     Frag = " [." + Chunk_PC + '-OBJ-Comp' + child_f + " ]"
                #     all_dict_frag[Frag] = Chunk_Posn
                #
                # elif chunk.isChild('punct', self.Pred_P):
                #     SYM = '|'
                #
                # else:
                ...
                #If Paninian dependency relations
